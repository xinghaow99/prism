...
STAT_COLLECTOR = StatCollector.from_env(method_name="xattn")
...
def Xattention_prefill(
    ...
    attn_mask = approx_simple_mask[:, :, :q_block_num, :k_block_num].contiguous()
    STAT_COLLECTOR.finish_select_timer(select_start, select_end)

    if block_sparse_attn_func is None:
        raise RuntimeError(
            "XAttention baseline requires `block_sparse_attn` but it is not installed."
        )
...
XATTN_STRIDE = int(os.environ.get("XATTN_STRIDE", 8))
XATTN_THRESHOLD = float(os.environ.get("XATTN_THRESHOLD", 0.9))
XATTN_BLOCK_SIZE = int(os.environ.get("BLOCK_SIZE", 128))
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    if q_len == 1 and past_key_values is not None:
        ...
        if STAT_COLLECTOR.collect_density:
            attn_output, block_mask = Xattention_prefill(
                query_states,
                k_rep,
                v_rep,
                stride=XATTN_STRIDE,
                threshold=XATTN_THRESHOLD,
