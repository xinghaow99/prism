...
def add_inference_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        help="Attention mode. 'flash' for dense baseline, 'xattn' for XAttention sparse, 'prism' for Prism sparse.",
    )
    group.add_argument(
        "--threshold",
        type=float,
        default=0.95,
        help="Top-p threshold for sparse attention block selection.",
    )
    group.add_argument(
        "--stride",
        type=int,
        default=8,
        help="Stride for XAttention estimation downsampling.",
    )
    group.add_argument(
        "--attn-warmup-steps",
        type=int,
        default=5,
        help="Number of initial denoising steps using dense attention before switching to sparse.",
    )
    group.add_argument(
        "--block-size",
        type=int,
