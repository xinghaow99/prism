...
class HunyuanVideoSampler(Inference):
    def __init__(
        ...
        # Store sparse attention config on the transformer for use during forward pass
        self.pipeline.transformer.attn_mode = getattr(args, "attn", "flash")
        self.pipeline.transformer.sparse_threshold = getattr(args, "threshold", 0.95)
        self.pipeline.transformer.sparse_stride = getattr(args, "stride", 8)
        self.pipeline.transformer.sparse_block_size = getattr(args, "block_size", 128)
        self.pipeline.transformer.attn_warmup_steps = getattr(args, "attn_warmup_steps", 5)
