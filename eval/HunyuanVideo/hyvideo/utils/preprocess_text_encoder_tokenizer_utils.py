...
def preprocess_text_encoder_tokenizer(args):
    ...
    # In newer transformers (>=5.0), the language model is at model.model.language_model
    lang_model = getattr(model, "language_model", None) or model.model.language_model
    lang_model.save_pretrained(
        f"{args.output_dir}"
    )
